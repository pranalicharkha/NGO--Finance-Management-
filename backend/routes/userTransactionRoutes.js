...
router.get("/transactions", async (req, res) => {
    try {
        ...
        const [incomeResults] = await promiseDb.query(`
            SELECT id, date, category, source, payment_method, amount, description, 'income' AS type
            FROM income
        `);

        const [expenseResults] = await promiseDb.query(`
            SELECT id, date, category, title, payment_method, amount, description, 'expense' AS type
            FROM expense
        `);
